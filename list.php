...
$query = sprintf("SELECT url, urls.id, last_update,
		yslow2.o as o,
		pagespeed.o as ps_o,
...
?>
	...
	?><tr>
		<?php if ($row['last_update']) { ?>
			<td style="text-align: right; padding-right: 1em"><a title="Time of last check for this URL" href="details/<?php echo urlencode($row['id']);?>/<?php echo htmlentities($row['url']); ?>"><?php echo htmlentities($row['last_update']); ?></a></td>
			...
			<td style="padding-left: 1em; overflow: hidden; white-space: nowrap;"><a href="details/<?php echo urlencode$row['id']);?>/<?php echo urlencode($row['url'])?>"><?php echo htmlentities(substr($row['url'], 0, 100))?><?php if (strlen($row['url']) > 100) { ?>...<?php } ?></a></td>
		<?php } else { ?>
