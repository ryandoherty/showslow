...
YAHOO.util.Event.onDOMReady(function() {
	...
	timeplot.loadXML('/details/events.php?url=' + encodeURIComponent(url) + '&ver=' + eventversion, showslowevents);
	if (ydataversion) {
		timeplot.loadText('/details/data.php?smooth=yes&subset=graph&profile=ydefault&url=' + encodeURIComponent(url) + '&ver=' + ydataversion, ",", eventSource2);
	}

	if (psdataversion) {
		timeplot.loadText('/details/data_pagespeed.php?smooth=yes&subset=graph&url=' + encodeURIComponent(url) + '&ver=' + psdataversion, ",", pagespeed);
	}

	if (dtdataversion) {
		timeplot.loadText('/details/data_dynatrace.php?smooth=yes&subset=graph&url=' + encodeURIComponent(url) + '&ver=' + dtdataversion, ",", dynatrace);
	}

	for (var name in metrics) {
		timeplot.loadText('/details/data_metric.php?smooth=yes&metric=' + name + '&url=' + encodeURIComponent(url), ",", metrics[name].source);
	}
})
