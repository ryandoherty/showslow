...
YAHOO.util.Event.onDOMReady(function() {
	...
	for (var url in data) {
		if (ranker == 'pagespeed') {
			timeplot.loadText('/details/data_pagespeed.php?smooth&subset=graph&url=' + encodeURIComponent(url) + '&ver=' + data[url].version, ",", data[url].eventsource);
		} else if (ranker == 'dynatrace') {
			timeplot.loadText('/details/data_dynatrace.php?smooth&subset=graph&url=' + encodeURIComponent(url) + '&ver=' + data[url].version, ",", data[url].eventsource);
		} else {
			timeplot.loadText('/details/data.php?smooth&subset=graph&profile=ydefault&url=' + encodeURIComponent(url) + '&ver=' + data[url].version, ",", data[url].eventsource);
		}
	}
